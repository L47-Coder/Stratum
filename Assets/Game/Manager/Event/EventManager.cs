...
internal sealed partial class EventManager : IEventManager
{
    ...
    private readonly Dictionary<string, EventManagerData> _managerDataDict = new();
    ...
    public void Publish<T>(T message)
    {
        foreach (var callback in GetCallbacks<Action<T>>(typeof(T)))
        {
            ...
            catch (Exception ex) { Debug.LogWarning($"同步事件处理失败: {ex.Message}"); }
        }
    }
    ...
    public async UniTask PublishAsync<T>(T message, int timeoutMs = 10000)
    {
        ...
        try
        {
            var publishTask = UniTask.WhenAll(callbacks.Select(callback => InvokeAsync(callback, message, groupSource.Token)).ToArray());
    ...
    private TCallback[] GetCallbacks<TCallback>(Type type)
    {
        lock (_lock)
        {
            ...
            return callbackDict.Values.OfType<TCallback>().ToArray();
        }
    }

    private static async UniTask InvokeAsync<T>(Func<T, CancellationToken, UniTask> callback, T message, CancellationToken token)
    {
        try { await callback(message, token); }
        catch (OperationCanceledException) { }
        catch (Exception ex) { Debug.LogWarning($"异步事件处理失败: {ex.Message}"); }
    }
}
