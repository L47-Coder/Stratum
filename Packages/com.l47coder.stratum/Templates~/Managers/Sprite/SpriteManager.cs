...
{
    private readonly Dictionary<string, SpriteManagerData> _managerDataDict = new(StringComparer.Ordinal);
    ...
    public async UniTask<ISpriteHandle> LoadSpriteAsync(string key)
    {
        ...
        var spriteCache = GetOrCreateCache(key, data.SpriteAddress);
    ...
    public async UniTask Release(ISpriteHandle handle)
    {
        ...
        if (spriteCache.AssetHandle == null) return;
    ...
    private SpriteCache GetOrCreateCache(string key, string spriteAddress)
    {
        ...
        UniTask.Void(async () =>
        {
            try
            {
                var assetHandle = await _assetManager.LoadAssetAsync<Sprite>(spriteAddress);
