...
{
    internal static class CoreCreationService
    {
        public static void CreateScript(CoreCreatorState state)
        {
            if (state == null || !state.IsValid) return;
            CreateScript(state.BuildPlan());
        }

        private static void CreateScript(CoreCreationPlan plan)
        {
            if (plan.ShouldCreateScript)
                WriteScript(plan);

            AssetDatabase.Refresh();
            CoreAssetIndex.Invalidate();
            Debug.Log($"[CoreCreationService] {plan.ClassName} ready.");
        }

        private static void WriteScript(CoreCreationPlan plan)
        {
    ...
    internal static class CoreAssetIndex
    {
        private static Dictionary<string, string> _scripts;

        static CoreAssetIndex()
        {
        ...
        public static string FindScript(string fileName) =>
            Find(ref _scripts, CoreCreatorState.RootAssetPath, fileName, ".cs");
