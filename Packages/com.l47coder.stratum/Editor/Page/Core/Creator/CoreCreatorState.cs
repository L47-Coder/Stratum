...
{
    internal sealed class CoreCreatorState : ICreatorState
    {
        public const string RootAssetPath = WorkbenchPaths.CoreRoot;
        ...
        public string GetPreviewSource() =>
            HasPreview ? CoreCreationService.BuildSource(ClassName) : string.Empty;

        public CoreCreationPlan BuildPlan()
        {
            RefreshExistingTargets();
            return new CoreCreationPlan(
                ClassName,
        ...
        private static bool TryNormalizeParentFolder(string parentAssetPath, out string normalizedParentPath, out string errorMessage)
        {
            ...
            { errorMessage = "Parent folder must be inside the Core root."; return false; }
        ...
        private void RefreshExistingTargets()
        {
            _existingScriptFilePath = ResolveExisting(ScriptFilePath,
                CoreAssetIndex.FindScript(Path.GetFileName(ScriptFilePath)));
