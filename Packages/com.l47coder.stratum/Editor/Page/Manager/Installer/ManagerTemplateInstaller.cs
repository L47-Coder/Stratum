    ...
    internal static class ManagerTemplateInstaller
    {
        ...
        public static bool IsPackageInstalled(string packageId)
        {
            ...
            var abs = Path.GetFullPath(Path.Combine(Application.dataPath, "..",
                $"{WorkbenchPaths.ManagerRoot}/{packageId}/{packageId}Manager.cs"));
        ...
        public static int InstallPackages(IList<string> packageIds)
        {
            ...
            foreach (var id in packageIds)
            {
                ...
                var sourcePath = $"{WorkbenchPaths.ManagerTemplatesFolder}/{id}";
                var destPath   = $"{WorkbenchPaths.ManagerRoot}/{id}";

                var sourceAbs = ResolveSourceAbsolute(id);
                if (string.IsNullOrEmpty(sourceAbs) || !Directory.Exists(sourceAbs))
                {
                    Debug.LogError($"[ManagerTemplateInstaller] Template folder missing: {sourcePath}");
                    continue;
                }
