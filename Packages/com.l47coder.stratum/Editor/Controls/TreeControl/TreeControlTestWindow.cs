...
{
    public sealed class TreeControlTestWindow : EditorWindow
    {
        ...
        [MenuItem("Tools/Dev Workbench/TreeControl Test")]
        private static void Open()
        {
            var w = GetWindow<TreeControlTestWindow>("TreeControl Test");
        ...
        private void DrawOptions()
        {
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                EditorGUILayout.HelpBox(
                    "树直接映射工程资源目录；新建 / 改名 / 删除 / 移动会写入磁盘。请在测试用子目录或备份工程中操作。忽略规则与 ListControl 相同；「隐藏扩展名」支持逗号、分号或换行，例如 .meta。",
                    MessageType.Warning);
            }
        }
