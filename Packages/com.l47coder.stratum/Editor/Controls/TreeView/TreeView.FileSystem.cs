...
namespace Stratum.Editor
{
    public sealed partial class TreeView
    {
        ...
        private void ExecuteDelete(TreeNode node)
        {
            ...
            if (!AssetDatabase.DeleteAsset(path)) { Debug.LogWarning($"[TreeView] Delete failed: {path}"); return; }
        ...
        private void ExecuteMove(string sourcePath, string targetDirPath)
        {
            ...
            if (tgt.StartsWith(src + "/", StringComparison.OrdinalIgnoreCase)) { Debug.LogWarning("[TreeView] Cannot move a folder into one of its own subfolders."); return; }
