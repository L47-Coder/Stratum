...
single-file Core, Manager, Component and ScriptableObject scripts.
...
- **Dev Workbench.** `Tools > Stratum > Dev Workbench` opens a reorderable
  editor window with four symmetric script generators (`Core`, `Manager`,
...
4. In `Core > Viewer`, generate shared lower-level classes used by the
   other generated assemblies.
5. In `Manager > Viewer`, select the root folder and use the creator panel
   on the right to generate a Manager script.
6. In `Component > Viewer`, generate any data-holder components your
   Managers depend on.
7. In `ScriptableObject > Viewer`, generate ScriptableObject types and
   create matching `.asset` instances directly from the Unity create menu.
8. Optionally import `EventManager`, `MessageManager` or `TaskManager` from
   `Manager > Import`.
9. In `Manager > Order`, arrange the runtime Manager boot order. Newly
   compiled Managers are added automatically; missing ones are pruned.
10. In your boot scene, add a `GameLifetimeScope` component and the generated
...
| Group | Tab | Purpose |
| --- | --- | --- |
| `Core` | `Viewer` | Browse `Assets/Game/Core` and generate dependency-light shared C# classes. |
